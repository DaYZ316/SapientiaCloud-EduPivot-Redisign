...
-- edu_student
UPDATE edu_student SET updated_at = created_at WHERE updated_at IS NULL;
DROP TRIGGER IF EXISTS set_updated_at ON edu_student;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON edu_student
    FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();

-- edu_teacher
UPDATE edu_teacher SET updated_at = created_at WHERE updated_at IS NULL;
DROP TRIGGER IF EXISTS set_updated_at ON edu_teacher;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON edu_teacher
    FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();
