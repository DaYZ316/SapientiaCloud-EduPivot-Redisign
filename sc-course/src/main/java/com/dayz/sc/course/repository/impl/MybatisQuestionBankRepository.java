...
public class MybatisQuestionBankRepository implements QuestionBankRepository {
    ...
    @Override
    public List<QuestionBank> findAll(int page, int size, UUID courseId, Integer bankType, String keyword) {
        LambdaQueryWrapper<QuestionBank> wrapper = new LambdaQueryWrapper<>();
        if (courseId != null) {
            wrapper.eq(QuestionBank::getCourseId, courseId);
        }
    ...
    @Override
    public long countAll(UUID courseId, Integer bankType, String keyword) {
        LambdaQueryWrapper<QuestionBank> wrapper = new LambdaQueryWrapper<>();
        if (courseId != null) {
            wrapper.eq(QuestionBank::getCourseId, courseId);
        }
