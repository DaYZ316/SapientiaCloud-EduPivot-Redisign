...
COMMENT
ON TABLE edu_practice_session IS '练习会话';
COMMENT
ON COLUMN edu_practice_session.session_type IS '会话类型: 0=顺序练习, 1=随机练习';
COMMENT
ON COLUMN edu_practice_session.status IS '状态: 0=进行中, 1=已完成, 2=已放弃';
...
COMMENT
ON TABLE edu_practice_answer IS '练习答案记录';
COMMENT
ON COLUMN edu_practice_answer.is_correct IS '是否正确: 0=错误, 1=正确';
