...
COMMENT ON TABLE edu_course_teacher IS 'Course-teacher relation table';
COMMENT ON COLUMN edu_course_teacher.course_id IS 'Course ID';
COMMENT ON COLUMN edu_course_teacher.teacher_id IS 'Teacher user ID';
COMMENT ON COLUMN edu_course_teacher.created_at IS 'Created time';
...
BEGIN
    IF EXISTS (
        ...
        WHERE c.deleted = 0
          ...
          AND jsonb_array_length(c.assistant_ids) > 0
        ON CONFLICT (course_id, teacher_id) DO NOTHING;
    END IF;
END $$;
