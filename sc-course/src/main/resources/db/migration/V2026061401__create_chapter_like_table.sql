...
COMMENT
ON TABLE  edu_chapter_like                IS '章节点赞关系表';
COMMENT
ON COLUMN edu_chapter_like.id             IS '主键';
COMMENT
ON COLUMN edu_chapter_like.chapter_id     IS '章节ID';
COMMENT
ON COLUMN edu_chapter_like.course_id      IS '课程ID（冗余，方便按课程查询）';
COMMENT
ON COLUMN edu_chapter_like.user_id        IS '用户ID';
COMMENT
ON COLUMN edu_chapter_like.created_at     IS '点赞时间';
