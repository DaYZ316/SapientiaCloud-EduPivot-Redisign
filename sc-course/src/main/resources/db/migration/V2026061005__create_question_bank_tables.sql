...
COMMENT ON TABLE edu_question_bank IS '课程题库';
COMMENT ON COLUMN edu_question_bank.bank_type IS '题库类型: 0=练习, 1=考试, 2=作业';
COMMENT ON COLUMN edu_question_bank.tags IS '标签列表 (JSON数组)';
COMMENT ON COLUMN edu_question_bank.difficulty IS '难度: 1=简单, 2=中等, 3=困难';
...
COMMENT ON TABLE edu_question IS '题目';
COMMENT ON COLUMN edu_question.question_type IS '题目类型: 0=单选, 1=多选, 2=判断, 3=填空, 4=简答';
COMMENT ON COLUMN edu_question.difficulty IS '难度: 1=简单, 2=中等, 3=困难';
COMMENT ON COLUMN edu_question.score IS '分值';
COMMENT ON COLUMN edu_question.estimated_time IS '预估答题时间 (分钟)';
COMMENT ON COLUMN edu_question.allow_partial_credit IS '允许部分得分: 0=否, 1=是';
COMMENT ON COLUMN edu_question.status IS '状态: 0=草稿, 1=已发布, 2=已禁用';
...
COMMENT ON TABLE edu_question_option IS '题目选项';
COMMENT ON COLUMN edu_question_option.option_label IS '选项标签: A, B, C, D 等';
COMMENT ON COLUMN edu_question_option.is_correct IS '是否正确: 0=错误, 1=正确';
COMMENT ON COLUMN edu_question_option.score IS '部分得分 (多选题)';
...
COMMENT ON TABLE edu_question_answer IS '题目答案';
COMMENT ON COLUMN edu_question_answer.answer_content IS '答案内容';
COMMENT ON COLUMN edu_question_answer.score IS '该空/该题得分';
COMMENT ON COLUMN edu_question_answer.sort_order IS '填空序号';
