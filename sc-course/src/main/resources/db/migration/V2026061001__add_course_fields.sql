...
-- 注释
COMMENT
ON COLUMN edu_course.assistant_ids IS '助教用户ID列表 (JSON数组)';
COMMENT
ON COLUMN edu_course.semester IS '开设学期 (例如: 2025秋季)';
COMMENT
ON COLUMN edu_course.location IS '上课地点';
COMMENT
ON COLUMN edu_course.course_type IS '课程类型: 0=必修, 1=选修';
COMMENT
ON COLUMN edu_course.is_public IS '是否公开: 0=仅课程成员, 1=公开';
