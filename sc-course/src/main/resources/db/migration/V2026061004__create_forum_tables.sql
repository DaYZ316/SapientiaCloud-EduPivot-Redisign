...
COMMENT ON TABLE edu_forum IS '课程论坛';
COMMENT ON COLUMN edu_forum.forum_type IS '论坛类型: 0=讨论, 1=问答, 2=作业, 3=公告';
COMMENT ON COLUMN edu_forum.allow_anonymous IS '允许匿名: 0=否, 1=是';
COMMENT ON COLUMN edu_forum.status IS '状态: 0=正常, 1=关闭, 2=维护中';
COMMENT ON COLUMN edu_forum.tags IS '标签列表 (JSON数组)';
...
COMMENT ON COLUMN edu_forum_post.is_anonymous IS '是否匿名: 0=实名, 1=匿名';
COMMENT ON COLUMN edu_forum_post.is_top IS '是否置顶: 0=否, 1=是';
COMMENT ON COLUMN edu_forum_post.is_essence IS '是否精华: 0=否, 1=是';
COMMENT ON COLUMN edu_forum_post.is_locked IS '是否锁定: 0=否, 1=是';
COMMENT ON COLUMN edu_forum_post.status IS '状态: 0=正常, 1=已删除, 2=审核中, 3=审核失败';
COMMENT ON COLUMN edu_forum_post.chapter_id IS '关联章节ID (可选)';
...
COMMENT ON COLUMN edu_forum_reply.reply_to_user_id IS '回复目标用户ID';
COMMENT ON COLUMN edu_forum_reply.is_accepted IS '是否被采纳: 0=否, 1=是 (问答模式)';
COMMENT ON COLUMN edu_forum_reply.floor_number IS '楼层号';
COMMENT ON COLUMN edu_forum_reply.status IS '状态: 0=正常, 1=已删除, 2=审核中, 3=审核失败';
