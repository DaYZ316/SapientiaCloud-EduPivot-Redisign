...
CREATE TABLE IF NOT EXISTS edu_class_session (
    ...
    live_room_name      VARCHAR(120) NOT NULL,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    deleted             SMALLINT    NOT NULL DEFAULT 0,
    CONSTRAINT chk_edu_class_session_time CHECK (scheduled_end_at > scheduled_start_at)
);
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON edu_class_session
    FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();

COMMENT ON TABLE edu_class_session IS '上课记录';
COMMENT ON COLUMN edu_class_session.published_at IS '发布时间，NULL 表示备课草稿';
COMMENT ON COLUMN edu_class_session.room_size IS '3D 教室规格: 0=小型, 1=中型, 2=大型, 3=超大型';
COMMENT ON COLUMN edu_class_session.live_room_name IS 'LiveKit 房间名称';
...
COMMENT ON TABLE edu_class_participant IS '课堂参与者';
COMMENT ON COLUMN edu_class_participant.role IS '参与者角色: 0=教师, 1=学生';
COMMENT ON COLUMN edu_class_participant.x IS '教室 2D 平面 X 坐标';
COMMENT ON COLUMN edu_class_participant.y IS '教室 2D 平面 Y 坐标';
COMMENT ON COLUMN edu_class_participant.z IS '预留 3D 坐标，当前固定为 0';
...
COMMENT ON TABLE edu_class_barrage IS '课堂弹幕';
COMMENT ON COLUMN edu_class_barrage.content IS '弹幕内容，最多 300 字符';
