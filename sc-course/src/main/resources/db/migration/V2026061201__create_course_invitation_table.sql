...
COMMENT
ON TABLE edu_course_invitation IS '课程助教邀请表';
COMMENT
ON COLUMN edu_course_invitation.course_id IS '课程ID';
COMMENT
ON COLUMN edu_course_invitation.inviter_id IS '邀请人ID（主讲教师）';
COMMENT
ON COLUMN edu_course_invitation.invitee_id IS '被邀请人ID（助教）';
COMMENT
ON COLUMN edu_course_invitation.status IS '邀请状态: 0=待处理, 1=已接受, 2=已拒绝, 3=已撤回';
COMMENT
ON COLUMN edu_course_invitation.message IS '邀请留言';
COMMENT
ON COLUMN edu_course_invitation.deleted IS '软删除: 0=正常, 1=已删除';
