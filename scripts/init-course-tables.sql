...
COMMENT ON COLUMN edu_course.id IS '课程ID';
COMMENT ON COLUMN edu_course.title IS '课程标题';
COMMENT ON COLUMN edu_course.description IS '课程描述';
COMMENT ON COLUMN edu_course.teacher_id IS '教师用户ID';
COMMENT ON COLUMN edu_course.level IS '难度等级: 1=初级, 2=中级, 3=高级';
COMMENT ON COLUMN edu_course.cover_url IS '封面图片URL';
COMMENT ON COLUMN edu_course.max_students IS '最大学生数, 0=不限';
COMMENT ON COLUMN edu_course.status IS '状态: 0=草稿, 1=已发布, 2=已归档';
COMMENT ON COLUMN edu_course.created_at IS '创建时间';
COMMENT ON COLUMN edu_course.updated_at IS '更新时间';
COMMENT ON COLUMN edu_course.deleted IS '逻辑删除: 0=正常, 1=已删除';
