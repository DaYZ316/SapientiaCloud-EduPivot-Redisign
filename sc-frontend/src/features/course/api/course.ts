...
export function getCourseEnrollments(courseId: string, page: number = 1, size: number = 10) {
    ...
    params.append('page', page.toString())
    params.append('size', size.toString())
    return request<PageResponse<Enrollment>>({
        method: 'GET',
        url: `/api/enrollments/course/${courseId}?${params.toString()}`
    })
}
...
export async function listCourseFiles(courseId: string, page: number = 1, size: number = 10): Promise<PageResponse<CourseFile>> {
    ...
    params.append('page', page.toString())
    params.append('size', size.toString())
    const response = await request<PageResponse<CourseFile> | CourseFile[]>({
        method: 'GET',
        url: `/api/courses/${courseId}/files?${params.toString()}`
    })
