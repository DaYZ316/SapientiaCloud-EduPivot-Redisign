    ...
    <div v-else-if="session && questions.length > 0" class="practice-container">
      ...
      <div v-if="currentQuestion" class="question-panel">
        ...
        <div class="practice-actions">
          <button v-if="!showFeedback" class="btn-submit" :disabled="!canSubmit" @click="handleSubmit">
...
import {createPracticeSession, getPracticeSession, submitAnswer, completePracticeSession} from '@/features/question-bank/api/practiceSession'
...
<style scoped>
.practice-view { max-width: 100%; }
.back-link { display: inline-flex; align-items: center; gap: 6px; padding: 0; border: none; background: none; color: var(--color-muted); font-family: var(--font-body); font-size: 13px; cursor: pointer; margin-bottom: 32px; }
...
.option-item:hover { border-color: var(--color-on-surface); }
...
.answer-textarea:focus { border-color: var(--color-on-surface); }
...
.btn-next { background: var(--color-primary); color: var(--color-on-primary); }
.btn-next:hover { background: var(--color-primary-soft); }
.start-container { display: grid; place-items: center; min-height: 400px; }
