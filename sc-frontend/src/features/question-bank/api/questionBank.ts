...
export function getQuestions(params: {questionBankId?: string; courseId?: string; page?: number; size?: number; questionType?: number; difficulty?: number; status?: number; keyword?: string}) {
  const searchParams = new URLSearchParams()
  ...
  if (params.keyword) searchParams.append('keyword', params.keyword)
  return request<PageResponse<Question>>({method: 'GET', url: "/api/question-banks/questions?" + searchParams.toString()})
}
