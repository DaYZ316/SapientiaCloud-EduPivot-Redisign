<template>
  <section class="teacher-live-panel">
    ...
    >
      ...
      <span>{{ teacher.liveSession ? t('success.dashboard.actions.enterClassroom3d') : t('success.dashboard.actions.maintainCourseContent') }}</span>
    </RouterLink>
  </section>

  <section class="metric-grid metric-grid--six teacher-metric-grid" :aria-label="t('success.dashboard.aria.teacherMetrics')">
    ...
    <div class="teacher-main-column">
      <article class="workbench-panel teacher-panel--today">
        ...
        <div class="session-list">
          ...
          >
            ...
            <span class="dashboard-status-chip" :class="{live: session.status === ClassSessionStatus.LIVE}">
      ...
      <article class="workbench-panel teacher-panel--courses">
        ...
        <div class="course-ops-grid">
          <RouterLink v-for="course in teacher.courses" :key="course.id" :to="`/courses/${course.id}/overview`" class="course-op">
    ...
    <div class="teacher-side-column">
      ...
      <article class="workbench-panel teacher-panel--capacity">
        ...
        <div class="capacity-list">
          ...
          <DashboardEmptyState v-if="teacherCapacityRanking.length === 0" :text="t('success.dashboard.empty.courseCapacity')" />
        </div>
      </article>
  ...
  <Teleport to="body">
    <div v-if="activeCourseAction" class="dashboard-course-picker-backdrop" @click.self="closeCoursePicker">
      <section class="dashboard-course-picker" role="dialog" aria-modal="true">
...
const teacherProductionActions = computed<TeacherProductionAction[]>(() => [
    {kind: 'link', label: t('success.dashboard.actions.createCourse'), to: '/teacher/courses?role=primary', icon: BookOpen},
    {kind: 'course', label: t('success.dashboard.actions.createChapter'), courseAction: 'createChapter', icon: PencilLine},
    {kind: 'course', label: t('success.dashboard.actions.uploadCourseware'), courseAction: 'uploadCourseware', icon: FileUp},
    {kind: 'course', label: t('success.dashboard.actions.createQuestionBank'), courseAction: 'createQuestionBank', icon: Library},
