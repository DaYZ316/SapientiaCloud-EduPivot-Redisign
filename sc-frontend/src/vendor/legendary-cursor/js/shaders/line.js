...
vec3 computeColor(vec2 uv, vec2 lineUvs, float mouseMixer) {
    ...
    vec2 textureUvs = fract(mouseDisplacedUvs + uvDistortion + uTextureOffset);
    vec3 col1 = texture2D(uTexture1, textureUvs ).rgb;
    vec3 col2 = texture2D(uTexture2, textureUvs ).rgb;
