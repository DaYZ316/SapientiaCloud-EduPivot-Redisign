...
LegendaryCursor.destroy = function () {
    ...
    sparkleMaterial = undefined;
    lightShaftMaterial = undefined;
    quadClearMaterial = undefined;
    timer = undefined;
    texture1 = undefined;
    texture2 = undefined;
    texture3 = undefined;
...
function updateAutoPilotPosition(time) {
    let pulse = Math.sin(time * 1.4) * 5;
    let orbitX = Math.cos(time * autoPilotSpeed) * (autoPilotRadius + pulse);
    let orbitY = Math.sin(time * autoPilotSpeed * 1.12) * (autoPilotRadius * 0.72);

    setCursorPosition(autoPilotCenter.x + orbitX, autoPilotCenter.y + orbitY);
    mouseDown = true;
}

function setCursorPosition(clientX, clientY) {
    let ux = (clientX / window.innerWidth) * 2 - 1;
    let uy = ((window.innerHeight - clientY) / window.innerHeight) * 2 - 1;
...
function updateTextureOffset(time, delta) {
    ...
    textureOffset.lerp(targetTextureOffset, Math.min(delta * 1.35, 1));
    lineMaterial.uniforms.uTextureOffset.value = textureOffset;
    sparkleMaterial.uniforms.uTextureOffset.value = textureOffset;
}
